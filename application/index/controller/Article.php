...
class Article extends Frontend
{
    ...
    public function index()
    {
        if($this->request->isAjax()){
            ...
            $lists=$model->alias('article')->with(['articletype','user'])->where(['user_id'=>$this->auth->getUser()->id,'article.status'=>0])->limit($offset,$limit)->order('id desc')->select();
            $total=$model->alias('article')->with(['articletype','user'])->where(['user_id'=>$this->auth->getUser()->id,'article.status'=>0])->count();
