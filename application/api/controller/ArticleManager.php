...
//        $where["articletype.status"]=["eq","显示"];
        ...
        $query=new Query();
        $data["rows"]=$query->table("fa_article")->alias("article")->cache(600)->field("article.*,articletype.name as articletype_name,user.username,user.avatar,1 as count_lihao, 1 as count_likong")
            ->where($where)
            ->whereExp('',$whereExp)
...
//        $where["articletype.status"]=["eq","显示"];
        ...
        foreach ($data["rows"] as $key=>&$value){
            $value["create_time"]=formart_time_kuaixun($value["create_time"]);
