...
//        $where["articletype.status"]=["eq","显示"];
        ...
        $query=new Query();
        $data["rows"]=$query->table("fa_article")->alias("article")->field("article.*,articletype.name as articletype_name,user.username,user.avatar,1 as count_lihao, 1 as count_likong")
            ->where($where)
            ->whereExp('',$whereExp)
